# Blog Analytics Dashboard

Source issue: https://github.com/LaceyPsych/laceypsych.github.io/issues/100

This dashboard tracks public performance and editorial learning across published
Generative Systems Psychology posts.

Analytics should guide presentation, sequencing, accessibility, and distribution.
Analytics should not distort Generative Systems Psychology.

If popularity and theoretical integrity conflict, theoretical integrity wins.

## Update Protocol

Update the individual blog issue first, then copy summary metrics here.

Use `docs/editorial-style-guide.md` for the style standard. A post should not be
marked editorially complete unless it preserves GSP fidelity, accountability,
public safety, and the generated-output pathway relevant to the article.

Suggested cadence:

- 24 hours after publication;
- 7 days after publication;
- 30 days after publication;
- monthly rollup by arc;
- whenever a meaningful reader response changes future editorial planning.

## Overall Metrics

| Metric | Current Total | Notes |
|---|---:|---|
| Published Posts | 4 | Counted from `content/posts/` on 2026-08-21. |
| Total Views | TBD | Requires analytics source. |
| Total Unique Visitors | TBD | Requires analytics source. |
| Current Followers | TBD | Requires platform-specific source. |
| Net Followers Gained | TBD | Requires baseline and current count. |
| Newsletter Subscribers | TBD | Requires newsletter provider. |
| Total Comments | TBD | Requires social/site collection. |
| Total Shares | TBD | Requires platform-specific source. |
| Total Backlinks | TBD | Requires backlink/search source. |

**Last Updated:** 2026-08-21

**Reporting Period:** Not yet connected to analytics provider

## Blog Performance

| # | Blog Title | Arc / Category | Published | 24-Hour Views | 7-Day Views | 30-Day Views | Total Views | Followers Gained | Comments | Shares | Backlinks | Status |
|---:|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 01 | A Person Is an Architecture Under Pressure | Theory Core | 2026-07-14 | TBD | TBD | TBD | TBD | TBD | TBD | TBD | TBD | Published |
| 02 | The Apology Draft That Became a Dissertation | Love, Safety, and Repair | 2026-07-26 | TBD | TBD | TBD | TBD | TBD | TBD | TBD | TBD | Published |
| 03 | Your Spreadsheet Is Trying To Protect You | Access and Dysregulation | 2026-08-05 | TBD | TBD | TBD | TBD | TBD | TBD | TBD | TBD | Published |
| 04 | Behavior Is the Smoke, Not the Fire | Theory Core | 2026-08-12 | TBD | TBD | TBD | TBD | TBD | TBD | TBD | TBD | Published |

## Editorial Style Compliance

| Blog Title | One Primary GSP Concept | GSP Pathway Visible | Accountability Preserved | Public Safety Clear | Editorial Status | Notes |
|---|---|---|---|---|---|---|
| A Person Is an Architecture Under Pressure | Yes | Yes | Yes | Yes | Aligned | Broad opening essay; Psychological Architecture remains construct-level language. |
| The Apology Draft That Became a Dissertation | Yes | Yes | Yes | Yes | Aligned | Relatedness, schemas, regulation, and access are located inside GSP. |
| Your Spreadsheet Is Trying To Protect You | Yes | Yes | Yes | Yes | Aligned | Appraisal, safety need, control-seeking, feedback, and Psychological Architecture are explicit. |
| Behavior Is the Smoke, Not the Fire | Yes | Yes | Yes | Yes | Aligned | Generated-output thesis is the central concept. |

For every new published blog, add a row here before marking the issue complete.

## Follower Tracking

| Date | Followers Before | Followers After | Net Change | Related Blog or Event | Notes |
|---|---:|---:|---:|---|---|
| TBD | TBD | TBD | TBD | TBD | Add after a baseline is chosen. |

## Traffic Sources

Traffic-source metrics are not available until an analytics source is connected.

| Source | Views | Percentage | Most Successful Post | Notes |
|---|---:|---:|---|---|
| Direct | TBD | TBD | TBD | TBD |
| Google / Search | TBD | TBD | TBD | TBD |
| LinkedIn | TBD | TBD | TBD | TBD |
| GitHub | TBD | TBD | TBD | TBD |
| Facebook | TBD | TBD | TBD | TBD |
| Reddit | TBD | TBD | TBD | TBD |
| Other | TBD | TBD | TBD | TBD |

## Social Performance

Social metrics are not available until platform posts and source URLs are
recorded.

| Blog | Platform | Post URL | Impressions | Clicks | Reactions | Comments | Shares | Followers Gained |
|---|---|---|---:|---:|---:|---:|---:|---:|
| TBD | LinkedIn | TBD | TBD | TBD | TBD | TBD | TBD | TBD |
| TBD | Facebook | TBD | TBD | TBD | TBD | TBD | TBD | TBD |
| TBD | Other | TBD | TBD | TBD | TBD | TBD | TBD | TBD |

## Top-Performing Posts

### Highest Total Views

1. TBD
2. TBD
3. TBD

### Highest Follower Growth

1. TBD
2. TBD
3. TBD

### Most Shared

1. TBD
2. TBD
3. TBD

### Most Discussed

1. TBD
2. TBD
3. TBD

## Performance by Arc

| Arc / Category | Published Posts | Total Views | Average Views | Followers Gained | Strongest Post |
|---|---:|---:|---:|---:|---|
| Theory Core | 2 | TBD | TBD | TBD | TBD |
| Access and Dysregulation | 1 | TBD | TBD | TBD | TBD |
| Love, Safety, and Repair | 1 | TBD | TBD | TBD | TBD |
| Parenting and Development | 0 | TBD | TBD | TBD | TBD |
| Work and Systems Psychology | 0 | TBD | TBD | TBD | TBD |
| Meaning and Self-Actualization | 0 | TBD | TBD | TBD | TBD |
| Protected Softness and Integration | 0 | TBD | TBD | TBD | TBD |

## Reader Response and Qualitative Feedback

Record useful reader reactions, questions, confusion, or repeated themes.

| Blog | Reader Feedback | What It Suggests | Possible Follow-Up |
|---|---|---|---|
| TBD | TBD | TBD | TBD |

## Editorial Findings

### What Appears To Be Working

- TBD after first metrics review.

### What May Need Adjustment

- TBD after first metrics review.

### Concepts Readers Want More Of

- TBD after first metrics review.

### Concepts Readers Appear To Misunderstand

- TBD after first metrics review.

## Future Experiments

Experiments may test presentation, sequencing, distribution, or accessibility.
They should not change GSP theory to chase engagement.

| Experiment | Posts Tested | Hypothesis | Result |
|---|---|---|---|
| Shorter opening scene | TBD | TBD | TBD |
| Fewer metaphors | TBD | TBD | TBD |
| More practical closing question | TBD | TBD | TBD |
| Clearer GSP pathway paragraph | TBD | TBD | TBD |
| Social post variation | TBD | TBD | TBD |

## New Post Metrics Template

Use this block in the post issue after publication, then copy results into this
dashboard.

```markdown
## Analytics

- Published URL:
- Published date:
- Arc / category:
- 24-hour views:
- 7-day views:
- 30-day views:
- Total views:
- Unique visitors:
- Followers before:
- Followers after:
- Net followers gained:
- Comments:
- Shares:
- Backlinks:
- Top traffic source:

## Editorial Learning

- Reader questions:
- Reader confusion:
- Resonant concept:
- Style note:
- Follow-up idea:
- Style guide still intact: yes/no
```
